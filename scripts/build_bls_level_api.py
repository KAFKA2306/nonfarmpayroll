...
Normal CI publishes only observations explicitly embedded in a committed audit
snapshot. A live BLS refresh is explicit and never runs implicitly on PRs.
"""
...
def parse_series(raw: bytes) -> list[dict[str, object]]:
    ...
    for row in reader:
        ...
        records.append({"series_id": SERIES_ID, "observation_date": f"{year:04d}-{month:02d}-01", "value_thousands": int(normalized["value"]), "preliminary": "P" in normalized.get("footnote_codes", "")})
    return _validate(records)


def records_from_snapshot(snapshot: dict[str, object]) -> list[dict[str, object]]:
    rows = []
    for key in ("previous_record", "latest_record"):
        source = snapshot[key]
        rows.append({"series_id": SERIES_ID, "observation_date": source["observation_date"], "value_thousands": int(source["value_thousands"]), "preliminary": bool(source["preliminary"])})
...
def build_payload(records: list[dict[str, object]], source_meta: dict[str, object]) -> dict[str, object]:
    ...
    return {"schema_version": 1, "series": {"series_id": SERIES_ID, "title": "Total nonfarm employment", "survey": "Current Employment Statistics", "unit": "thousands of persons", "seasonal_adjustment": "seasonally adjusted"}, "source": source_meta, "record_count": len(records), "first_observation": records[0]["observation_date"], "last_observation": records[-1]["observation_date"], "records": records, "warning": "Latest available level observations at retrieval time; not release-vintage history and not valid for revision-stage analysis."}
...
def write_outputs(payload: dict[str, object], output_dir: Path) -> None:
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=FIELDS); writer.writeheader(); writer.writerows(payload["records"])
    manifest = {"schema_version": 1, "analysis_available": False, "revision_vintage_available": False, "level_series_available": True, "series_id": SERIES_ID, "record_count": payload["record_count"], "first_observation": payload["first_observation"], "last_observation": payload["last_observation"], "retrieved_at_utc": payload["source"]["retrieved_at_utc"], "source_sha256": payload["source"]["source_sha256"], "files": {"total-nonfarm.json": {"bytes": len(json_bytes), "sha256": hashlib.sha256(json_bytes).hexdigest()}, "total-nonfarm.csv": {"bytes": csv_path.stat().st_size, "sha256": hashlib.sha256(csv_path.read_bytes()).hexdigest()}}}
...
def main() -> int:
    ...
    parser.add_argument("--refresh", action="store_true", help="Fetch BLS live; intended for explicit data refresh only")
    args = parser.parse_args()
    snapshot = json.loads(args.snapshot.read_text(encoding="utf-8"))
    if args.refresh:
        raw = fetch_source(); records = parse_series(raw)
    ...
    payload = build_payload(records, source_meta); write_outputs(payload, args.output_dir)
    print(f"published {payload['record_count']} verified observations: {payload['first_observation']}..{payload['last_observation']}")
    return 0
